...
float heu::Heuristic_Defensive2::operator()(board::GameBoard board, player::PlayerSide side){
    ...
    if(side == player::Player0){
        evaluation = ( 2 * Player0Left + 2 * Player0MostFrontDistance + 3 * Player0AverageDistanceFromHome )
        -( 2 * Player1Left + 5 * Player1MostFrontDistance + 2 * Player1AverageDistanceFromHome )
        + ( rand() / double(RAND_MAX) );
    }
    else if(side == player::Player1){
        evaluation = ( 2 * Player1Left + 2 * Player1MostFrontDistance + 3 * Player1AverageDistanceFromHome )
